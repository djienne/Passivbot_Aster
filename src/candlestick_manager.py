...
class CandlestickManager:
    ...
    def _symbol_dir(
        ...
        if tf_dir == "1m":
            coin = self._legacy_coin_from_symbol(symbol) or _sanitize_symbol(symbol)
            return str(Path(self.cache_dir) / f"ohlcvs_{ex}" / coin)
        sym = _sanitize_symbol(symbol)
        return str(Path(self.cache_dir) / f"ohlcvs_{ex}" / tf_dir / sym)
    ...
    def _fetch_lock_path(self, symbol: str, timeframe: str) -> str:
        lock_dir = os.path.join(self._symbol_dir(symbol, timeframe=timeframe), "locks")
    ...
    def _load_from_disk(
        ...
        *,
        ...
        Canonical raw shard path: `{cache_dir}/ohlcvs_{exchange}/{coin}/YYYY-MM-DD.npy`
        """
        try:
            tf_norm = self._normalize_timeframe_arg(timeframe, tf)
            ...
            for key, (day_start, day_end) in days.items():
                ...
                if primary_path is None:
                    continue
                load_keys.append((key, primary_path))
            if not load_keys:
                return
            self._log(
                "debug",
