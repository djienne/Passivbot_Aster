...
class OHLCVManager:
    ...
    async def download_ohlcvs(self, coin):
        ...
        if self.exchange == "binanceusdm":
            ...
            if self.cc is None:
                self.load_cc()
            await self.download_ohlcvs_aster(coin)
    ...
    async def load_markets(self):
        self.load_cc()
        self.markets = await load_markets(self.exchange, verbose=False, cc=self.cc)
    ...
    async def download_ohlcvs_aster(self, coin: str):
        missing_days = await self.get_missing_days_ohlcvs(coin)
        if not missing_days:
            return
        if self.cc is None:
        ...
        for day in missing_days:
            await self.check_rate_limit()
            tasks.append(asyncio.create_task(self.fetch_and_save_day_aster(symbol, day, dirpath)))
        for task in tasks:
            await task

    async def fetch_and_save_day_aster(self, symbol: str, day: str, dirpath: str):
        fpath = os.path.join(dirpath, f"{day}.npy")
        start_ts_day = date_to_ts(day)
        end_ts_day = start_ts_day + 24 * 60 * 60 * 1000
        ohlcvs = await self.cc.fetch_ohlcv(
        ...
        if not ohlcvs:
            if self.verbose:
                logging.info(f"No data returned for Aster {symbol} {day}")
            return
        df_day = pd.DataFrame(
        ...
        if len(df_day) == 1440:
            dump_daily_ohlcv_data(ensure_millis_df(df_day), fpath, start_ts_day)
            if self.verbose:
                logging.info(f"aster Dumped daily OHLCV data for {symbol} to {fpath}")
