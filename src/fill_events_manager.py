...
class AsterFetcher(BaseFetcher):
    """Fetch fill events for Aster using per-symbol userTrades plus order enrichment."""

    def __init__(self, api, *, symbols: list[str], symbol_resolver: Callable[[Optional[str]], str]) -> None:
        self.api = api
        self.symbols = symbols
        self.symbol_resolver = symbol_resolver
        self._order_detail_cache: Dict[Tuple[str, str], Tuple[str, str]] = {}

    @staticmethod
    def _normalize_position_side(side: str, raw_trade: dict, pnl: float) -> str:
    ...
    async def _get_order_details(self, symbol: str, order_id: Optional[str]) -> Tuple[str, str]:
        if not order_id:
            return "", "unknown"
        key = (symbol, str(order_id))
        if key in self._order_detail_cache:
            return self._order_detail_cache[key]
        client_oid = ""
        pb_type = "unknown"
        try:
            orders = await self.api.fetch_all_orders(symbol, order_id=int(order_id), limit=1)
        except Exception:
            orders = []
        if orders:
            info = orders[0]
            client_oid = str(info.get("clientOrderId") or info.get("origClientOrderId") or "")
            if client_oid:
                pb_type = custom_id_to_snake(client_oid)
        self._order_detail_cache[key] = (client_oid, pb_type)
        return client_oid, pb_type
    ...
    async def fetch(
        ...
        events: Dict[str, Dict[str, object]] = {}
        symbols = [self.symbol_resolver(sym) for sym in self.symbols if self.symbol_resolver(sym)]
        for symbol in sorted(set(symbols)):
            try:
                ...
                logger.debug("AsterFetcher.fetch_user_trades failed for %s: %s", symbol, exc)
                continue
            batch_events: List[Dict[str, object]] = []
            for raw_trade in trades:
                raw_symbol = str(raw_trade.get("symbol") or "")
                trade_id = raw_trade.get("id") or raw_trade.get("tradeId")
                if trade_id in (None, ""):
                    continue
                event_id = f"{raw_symbol}:{trade_id}"
                client_oid, pb_type = detail_cache.get(event_id, ("", "unknown"))
                order_id = raw_trade.get("orderId") or raw_trade.get("orderID")
                if not client_oid and order_id not in (None, ""):
                    client_oid, pb_type = await self._get_order_details(symbol, str(order_id))
                if not client_oid:
                    client_oid = str(raw_trade.get("clientOrderId") or "")
                    if client_oid:
                        pb_type = custom_id_to_snake(client_oid)
                pnl = float(raw_trade.get("realizedPnl") or raw_trade.get("realizedPNL") or raw_trade.get("pnl") or raw_trade.get("profit") or 0.0)
                side = str(raw_trade.get("side") or "").lower()
                if not side:
                    side = "buy" if bool(raw_trade.get("buyer")) else "sell"
                ts = int(raw_trade.get("time") or raw_trade.get("timestamp") or 0)
                event = {
                    "id": event_id,
                    "timestamp": ts,
                    "datetime": ts_to_date(ts) if ts else "",
                    "symbol": self.symbol_resolver(raw_symbol),
                    "side": side,
                    "qty": float(raw_trade.get("qty") or raw_trade.get("origQty") or raw_trade.get("amount") or 0.0),
                    "price": float(raw_trade.get("price") or 0.0),
                    "pnl": pnl,
                    "fees": {
                        "currency": str(raw_trade.get("commissionAsset") or raw_trade.get("feeAsset") or ""),
                        "cost": abs(float(raw_trade.get("commission") or raw_trade.get("fee") or 0.0)),
                    },
                    "pb_order_type": pb_type or "unknown",
                    "position_side": self._normalize_position_side(side, raw_trade, pnl),
                    "client_order_id": client_oid,
                    "raw": [{"source": "aster_user_trades", "data": raw_trade}],
                    "c_mult": 1.0,
                }
                events[event_id] = event
                if client_oid:
                    detail_cache[event_id] = (client_oid, pb_type or "unknown")
                batch_events.append(event)
            if on_batch and batch_events:
                on_batch(batch_events)
        return sorted(events.values(), key=lambda ev: ev["timestamp"])
...
def _build_fetcher_for_bot(bot, symbols: List[str]) -> BaseFetcher:
    ...
    if exchange == "aster":
        resolved_symbols = [resolver(sym) for sym in symbols]
        return AsterFetcher(
            api=bot.cca,
            symbols=[sym for sym in resolved_symbols if sym],
