...
async def _prepare_hlcvs_lighter(config, coins, effective_start_ts, requested_start_ts, end_ts):
    ...
    Reads from historical_data/ohlcvs_lighter/{COIN}/YYYY-MM-DD.npy (shape 1440x6: ts,o,h,l,c,v).
    Returns (mss, timestamps, unified_array, btc_usd_prices).
    """
    data_dir = Path(
        config.get("backtest", {}).get("lighter_data_dir", "historical_data/ohlcvs_lighter")
    )
    interval_ms = 60_000
