...
class Passivbot(ExchangeInterface):
    def __init__(self, config: dict):
        ...
        # Lighter has lower API rate limits; cap concurrency
        if self.exchange == "lighter":
            cm_kwargs["max_concurrent_requests"] = 2
    ...
    def create_ccxt_sessions(self):
        ...
        Subclasses are expected to replace this with exchange-specific setup.
        """
        raise NotImplementedError("create_ccxt_sessions must be implemented by exchange subclass")
    ...
    async def warmup_candles_staggered(
        ...
        if concurrency is None:
            ...
            if cfg_concurrency > 0:
                ...
                # Exchange-specific defaults: Hyperliquid/Lighter have stricter rate limits
                exchange_lower = self.exchange.lower() if self.exchange else ""
                if exchange_lower in ("hyperliquid", "lighter"):
                    concurrency = 2
                else:
                    concurrency = 8
    ...
    async def fetch_positions(self):
        """Fetch normalized positions from the exchange."""
        raise NotImplementedError("fetch_positions must be implemented by exchange subclass")

    async def fetch_open_orders(self, symbol=None):
        """Fetch normalized open orders from the exchange."""
        raise NotImplementedError("fetch_open_orders must be implemented by exchange subclass")

    async def fetch_tickers(self):
        """Fetch normalized tickers from the exchange."""
        raise NotImplementedError("fetch_tickers must be implemented by exchange subclass")

    async def fetch_ohlcv(self, symbol, timeframe="1m"):
        """Fetch OHLCV candles for a symbol and timeframe."""
        raise NotImplementedError("fetch_ohlcv must be implemented by exchange subclass")

    async def fetch_ohlcvs_1m(self, symbol, since=None, limit=None):
        """Fetch 1m OHLCV candles for EMA and downloader flows."""
        raise NotImplementedError("fetch_ohlcvs_1m must be implemented by exchange subclass")

    async def fetch_pnls(self, start_time=None, end_time=None, limit=None):
        """Fetch realized PnL / trade history for the exchange."""
        raise NotImplementedError("fetch_pnls must be implemented by exchange subclass")

    def get_order_execution_params(self, order: dict) -> dict:
        """Return exchange-specific order params for execution."""
        return self._build_order_params(order)

    async def watch_orders(self):
        """Background order-watch loop. Exchanges without WS support may no-op."""
        return
