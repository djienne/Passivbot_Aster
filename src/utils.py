...
def filter_markets(markets: dict, exchange: str, quote=None, verbose=False) -> (dict, dict, dict):
    ...
    for k, v in markets.items():
        ...
        elif exchange == "lighter" and not v.get("active", True):
            ineligible[k] = v
            reasons[k] = f"ineligible on {exchange}"
        else:
            eligible[k] = v
...
async def load_markets(
    ...
    # Aster is a custom connector in this repo, so fetch markets without CCXT.
    if ex == "aster":
        markets_path = os.path.join("caches", ex, "markets.json")
        try:
            if os.path.exists(markets_path):
        ...
        from exchanges.aster_rest import AsterRestClient

        owned_client = cc is None
        if owned_client:
            cc = AsterRestClient()
        try:
...
def load_ccxt_instance(exchange_id: str, enable_rate_limit: bool = True, timeout_ms: int = 60_000):
    ...
    if ex == "aster":
        ...
        client = AsterRestClient(timeout_ms=timeout_ms)
        client.options["enableRateLimit"] = bool(enable_rate_limit)
        return client
    try:
        cc = getattr(ccxt, ex)(
            {
