...
@pytest.mark.asyncio
async def test_loads_canonical_historical_data_shard(tmp_path, monkeypatch):
    ...
    cm = CandlestickManager(exchange=None, exchange_name="binanceusdm", cache_dir=str(tmp_path / "historical_data"))
...
@pytest.mark.asyncio
async def test_canonical_leading_minutes_are_not_backfilled_by_default(tmp_path, monkeypatch):
...
@pytest.mark.asyncio
async def test_canonical_leading_minutes_are_backfilled_when_requested(tmp_path, monkeypatch):
...
def test_partial_but_continuous_day_still_allows_canonical_write(tmp_path, monkeypatch):
    """A partial existing day should not block the canonical shard write."""
    monkeypatch.chdir(tmp_path)
    ...
    # With canonical-only storage, this MUST be written.
