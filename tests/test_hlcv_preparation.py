...
class TestHLCVManagerBasics:
    ...
    @pytest.mark.asyncio
    async def test_load_cc_aster_uses_custom_client(self, tmp_path):
        """Aster should use the custom exchange client path, not raw CCXT lookup."""
        aster_client = MagicMock()
        aster_client.id = "aster"
        aster_client.options = {}
        aster_client.close = AsyncMock()

        with patch("hlcv_preparation.load_ccxt_instance", return_value=aster_client):
            om = HLCVManager(
                exchange="aster",
                start_date="2024-01-01",
                end_date="2024-01-02",
            ...
            assert om.cc is aster_client
            assert om.cm is not None
            assert isinstance(om.cm, CandlestickManager)

    @pytest.mark.asyncio
    async def test_load_markets_reuses_existing_exchange_client(self, mock_exchange, mock_markets):
        """load_markets should pass the existing exchange client through to utils.load_markets."""
        with patch("hlcv_preparation.load_ccxt_instance", return_value=mock_exchange):
            om = HLCVManager(
                exchange="aster",
                start_date="2024-01-01",
            ...
            om.load_cc()
            with patch("hlcv_preparation.load_markets", new=AsyncMock(return_value=mock_markets)) as lm:
                await om.load_markets()
            assert om.markets == mock_markets
            assert lm.await_args.kwargs["cc"] is mock_exchange
